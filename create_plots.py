...
for ind, fname in enumerate(fnames):
    ...
    for m_i, m in enumerate(metrics):
        ...
        for i in range(3):
            for j, row in enumerate(new_results[usages[i], :, m_i]):
                ...
                ax[i].plot(
                    ...
                    c=colors[i][j],
                    ls=ls[i][j],
                    label="%s\n$^{%.3f}$" % (labels[usages[i][j]], np.mean(row)),
                    lw=lw[i][j],
                )
            ax[i].set_title(titles[i])
            ax[i].set_ylim(0, 1)
            ax[i].set_xlim(0, 100 - 1)
            ax[i].legend(ncol=len(usages[i]) // 2, frameon=False, fontsize=10)
        ...
        plt.savefig("figures/eps/%s.eps" % (fname + "_" + m))
    ...
    for i in range(3):
        ...
        ax[i].set_title("%s metric" % metrics[i])
        ax[i].set_ylim(0, 1)
        ax[i].legend(ncol=4, frameon=False)
        ax[i].spines["right"].set_color("none")
        ax[i].spines["top"].set_color("none")
        ax[i].grid(ls=":")
    ...
    fig.suptitle(ftitles[fname] + ".")
    fig.subplots_adjust(top=0.93, bottom=0.03, left=0.06, right=0.97)

    # plt.tight_layout()
    plt.savefig("bar.png")
    plt.savefig("figures/%s.png" % fname)
    plt.savefig("figures/eps/%s.eps" % fname)
